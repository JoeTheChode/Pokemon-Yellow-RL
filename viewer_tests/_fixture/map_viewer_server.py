...
class AgentFeed:
    ...
    def _player_card_payload(self):
        ...
        Every worker resets onto the same frontier state, so name/money/badges
        describe one run; play time differs only by how far into its own
        episode each worker is, so the furthest gives a clock that ticks
        instead of sawtoothing 96 ways.

        Badges take a *consensus*, not a bitwise OR. Workers that read WRAM
        mid-transition report a garbage byte, and OR is the worst aggregate
        for that -- a single bad reader permanently lights a badge. Observed
        2026-08-16: 62 workers read 3 (Boulder+Cascade, correct) while 34 read
        57 during a map transition; 57|3 = 59 lit badges 1,2,4,5,6, i.e.
        Rainbow/Soul/Marsh without Thunder, which no route can produce because
        Vermilion Gym is behind a Cut tree. Twenty seconds later all 96 read 3.
        The majority value is right there and a stray reader cannot outvote it.
        """
        ...
        badge_agreement = (
            flag_votes[badge_flags] / sum(flag_votes.values()) if flag_votes else None
        )
        names = [
        ...
        if not names and not badge_flags and not play_time_seconds:
            # Trainer predates the card fields; show nothing rather than a card
            # of zeroes that reads like a wiped save.
            return None
        return {
            ...
            # Below 1.0 means some workers disagree -- the signal that tells
            # "the run earned a badge" apart from "somebody misread".
            'badge_agreement': badge_agreement,
            'badges': [
