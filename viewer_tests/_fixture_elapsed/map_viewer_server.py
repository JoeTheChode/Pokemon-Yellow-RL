...
# How many worst objectives the payload pre-ranks by wall-clock dwell.
QUEST_TIME_SINK_ROWS = 8
...
class AgentFeed:
    def __init__(self):
        ...
        # phase -> {'name', 'clears', 'best', 'last', 'samples',
        #           'best_run_step', 'attempt_clears', 'best_secs',
        #           'last_secs', 'secs_samples'}
        self.quest_splits = {}
        ...
        return json.dumps(
            {
                ...
                # Pre-ranked so the audit does not depend on the client
                # sorting a 302-row table it renders in phase order.
                'time_sinks': [
                    {
                        'phase': row['phase'],
                        'name': row['name'],
                        ...
                        'share': (
                            round(100.0 * row['dwell_secs'] / sum(dwelt), 1)
                            if sum(dwelt) > 0 else None
                        ),
                    }
                    for row in sorted(
                        (r for r in rows if r['dwell_secs']),
                        key=lambda r: r['dwell_secs'],
                        reverse=True,
                    )[:QUEST_TIME_SINK_ROWS]
                ],
                'rows': rows,
